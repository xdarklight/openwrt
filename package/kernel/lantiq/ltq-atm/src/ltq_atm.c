...
static irqreturn_t mailbox_irq_handler(int irq, void *dev_id)
{
	...
	disable_irq_nosync(ppe_mailbox_irq);
	tasklet_schedule(&g_dma_tasklet);
...
static int ltq_atm_probe(struct platform_device *pdev)
{
	...
	ppe_mailbox_irq = platform_get_irq(pdev, 0);
	ret = devm_request_irq(&pdev->dev, ppe_mailbox_irq,
			       mailbox_irq_handler, 0, NULL, &g_atm_priv_data);
	if ( ret ) {
		if ( ret == -EBUSY ) {
